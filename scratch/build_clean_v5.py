...
sample_data_code = f"const DEFAULT_SAMPLE_DATASET = {sample_games_json};"
        ...
        <div class="status-bar">
            ...
            <div class="card" style="display: flex; flex-direction: column; justify-content: space-between;">
                <div>
                    ...
                    <div id="sparklineChart" class="sparkline-container"></div>
                </div>
    ...
    <script>
        ...
        function renderSparklineChart(convMsgs) {
            ...
            for (let t = 0; t < totalTurns; t++) {
                ...
                let cpEval = 0;
                if (assistantMsg) {
                    const match = assistantMsg.content.match(new RegExp('CENTIPAWN TỔNG HỢP:\\\\s*([+-]?\\\\d+)cp', 'i'));
        ...
        function renderCandidatesTable(thoughtText) {
            ...
            const candReg = new RegExp('\\\\[25/32\\\\][^\\\\n]+(?:\\\\n(?!\\\\[26/32\\\\])[^\\\\n]+)*');
            const candSection = thoughtText.match(candReg);
            if (!candSection) {
            ...
            const candLineReg = new RegExp('\\\\+?\\\\s*Ứng viên\\\\s*\\\\d+:\\\\s*([a-i][0-9][a-i][0-9])?\\\\s*—?\\\\s*([^★\\\\n(]+)?(?:\\\\(([^)]+)\\\\))?');

            lines.forEach((line, idx) => {
                const match = line.match(candLineReg);
...
print("✅ Clean v5 HTML written successfully!")
