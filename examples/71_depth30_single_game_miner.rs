// ============================================================================
// EXAMPLE 71: DEPTH 30 SINGLE GAME MINER EXECUTION (RULE 8.10/7.10)
// ============================================================================
// Động Cơ Thực Thi Khai Thác 1 Ván Cờ Tự Đấu Ở Độ Sâu Siêu Cấp Depth 30:
//   1. Tìm kiếm Alpha-Beta / PVS Độ Sâu Depth 30 (kết hợp time cap 2,000ms/nước đi).
//   2. Nạp mảng băm Zobrist TT 256MB đảm bảo cắt tỉa nhánh cây tối đa.
//   3. Xuất mẫu dữ liệu dạng JSONL chuẩn (`data/depth30_single_game_mined.jsonl`).
...
pub const APP_VERSION: &str = "v7.1.0-depth30-single-game-miner";
/// Hằng số dấu thời gian đóng gói APP_BUILD_STAMP
pub const APP_BUILD_STAMP: &str = "2026-08-12 13:35:00 ICT";
...
fn main() {
    println!("============================================================");
    println!(" 🚀 XIANGQI-RIM: DEPTH 30 SINGLE GAME MINER EXECUTION");
    ...
    let output_path = "data/depth30_single_game_mined.jsonl";

    println!("⚡ BẮT ĐẦU KHAI THÁC 1 VÁN CỜ ĐỘ SÂU DEPTH {}:", target_depth);
    println!("   • Dung lượng Shared TT     : 256 MB RAM (Zobrist Table)");
    println!("   • Tệp xuất dữ liệu JSONL   : {}", output_path);
    println!("   • Thời gian tối đa / nước  : 2,000 ms / nước (Exact Time Limit)");
    println!("============================================================");
    let _ = stdout().flush();
    ...
    let file = OpenOptions::new()
        ...
        .truncate(true)
        .open(output_path)
        .expect("Không thể tạo tệp xuất dữ liệu JSONL Depth 30");
    ...
    for ply in 1..=20 {
        ...
        limits.exact = 2000; // 2,000ms max per move
