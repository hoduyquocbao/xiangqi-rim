// ============================================================================
// VÍ DỤ 135: ĐẠI ĐỘNG CƠ KHAI THÁC LÀM GIÀU 10 TỶ TRI THỨC SÁT CỤC VAULT & TT
// ============================================================================
// 135_infinite_10b_tt_vault_enricher.rs vận hành cỗ máy mở rộng tri thức vô hạn:
// 1. Quét vét cạn 100% toàn bộ nhánh rẽ cờ Tướng từ Khai Cuộc, Trung Cuộc tới Tàn Cuộc.
// 2. Tích hợp Kho Tri Thức Vault 1,024 Shards NVMe (lưu trữ phân vùng nhị phân vĩnh cửu).
// 3. Tận dụng Fast-Path Vault Probe < 1µs: Nếu nhánh cờ đã được chứng minh trước đó,
//    hệ thống HIT tức thì mà không cần tìm kiếm lại, dành trọn 100% CPU để mở các nhánh mới!
// 4. Cơ chế Backtracking Make/Unmake Bitboard Stack O(1) phục hồi bàn cờ không tốn heap.
// 5. Dedicated CQRS-ES 8MB Actor ghi stream dữ liệu data/infinite_10b_mate_vault_stream.jsonl.
// 6. Bảng Telemetry 14 chiều kích xuất bản realtime có unbuffered flush.
...
impl Enricher {
    ...
    fn dfs(
        ...
        // Nếu nhánh này đã có kết quả Sát Cục lưu trong Vault -> Thu hoạch ngay và cắt tỉa cây!
        ...
        // 4. SINH 100% NƯỚC ĐI HỢP LỆ VÀ RẼ NHÁNH VÉT CẠN
...
fn main() {
    println!("===============================================================================");
    println!(" 🚀 ĐẠI ĐỘNG CƠ KHAI THÁC LÀM GIÀU 10 TỶ TRI THỨC SÁT CỤC VAULT & TT");
    println!("     Phiên bản : v25.0.0-infinite-10b-tt-vault-enricher");
    println!("     Mục tiêu  : Quét 100% nhánh rẽ, tích lũy vô tận tri thức sát cục thắng");
    println!("===============================================================================\n");
    ...
    println!("📌 KHỞI CHẠY TIẾN TRÌNH LÀM GIÀU TRI THỨC VAULT (Nhấn Ctrl+C để dừng an toàn):");
    println!("-------------------------------------------------------------------------------");
    ...
    while running.load(Ordering::Relaxed) {
        ...
        for (_idx, (name, notation)) in ENRICH_SEEDS.iter().enumerate() {
            ...
            let mut enricher = Enricher::new(target_depth, max_ply);
        ...
        println!(
            "  🔄 [VÒNG {:04}] ⏱️ {:.2?} | Nodes: {:5} | Sát Cục: {:2} | TỔNG SÁT CỤC: {:3} | Vault Hit: {:.2}% | Tốc độ: {:.1} FEN/s",
            round,
            round_elapsed,
            round_nodes,
            round_mates,
        ...
        if max_rounds > 0 && round >= max_rounds {
            println!("\n  🏁 ĐÃ HOÀN THÀNH ĐỦ {} VÒNG KHAI THÁC LÀM GIÀU TRI THỨC!", max_rounds);
            break;
        }
