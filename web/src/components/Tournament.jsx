...
export default function Tournament({ onClose }) {
  ...
  // Fetch match data JSONL from backend/public
  useEffect(() => {
    fetch('/match_d30_vs_d60.jsonl')
      .then((res) => res.text())
      .then((text) => {
        ...
        if (parsed.length > 0) {
          ...
          // Fallback sample tournament data if file is empty during live generation
          setMatchData([
            { ply: 1, side: 'RED (D30)', fen: startFen, best_move: 'b2e2', score: 0 },
            { ply: 2, side: 'BLACK (D60)', fen: 'rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR b - - 0 1', best_move: 'h7e7', score: 0 },
            { ply: 3, side: 'RED (D30)', fen: 'rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR w - - 0 1', best_move: 'a0a1', score: 4 },
        ...
        <div className="p-4 border-b border-gold/20 flex items-center justify-between bg-obsidian-header">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs font-mono text-gold/60">RED AI (DEPTH 30) VS BLACK AI (DEPTH 60)</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5 grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4">
            ...
            <div className="bg-obsidian/80 p-4 rounded-xl border border-gold/20 flex-1 flex flex-col gap-2 min-h-[220px]">
              ...
              <div className="flex-1 overflow-y-auto flex flex-col gap-1 pr-1 text-xs font-mono">
