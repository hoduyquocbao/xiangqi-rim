# ============================================================================
# SCRIPT 28: HIGH-THROUGHPUT STREAMING TRAIN & QUANTIZE NNUE GEN 8 (100M FEN)
# ============================================================================
# Kịch bản PyTorch GPU huấn luyện mô hình NNUE Gen 8 quy mô siêu lớn 100M FEN.
...
# ----------------------------------------------------------------------------
# 1. ĐỊNH NGHĨA KIẾN TRÚC MẠNG THẦN KINH NNUE GEN 8 (HALFKAV2_HM)
# ----------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------
# 3. KỊCH BẢN THỰC THI HUẤN LUYỆN CHÍNH SIÊU QUY MÔ (MAIN 100M TRAINING LOOP)
# ----------------------------------------------------------------------------
def main():
    print("============================================================", flush=True)
    print(" HUẤN LUYỆN PYTORCH NNUE GEN 8 SIÊU QUY MÔ 100M FEN TRÊN GPU", flush=True)
    print("============================================================", flush=True)
    ...
    print("--> Bắt đầu huấn luyện PyTorch NNUE Gen 8 trong 20 epochs...", flush=True)
    ...
    for epoch in range(1, 21):
        ...
        print(f"  Epoch [{epoch:2d}/20] | Loss: {loss.item():.6f} | LR: {lr_curr:.6f} | Elapsed: {time.time() - start_time:.2f}s", flush=True)
        
    output_weights_path = "data/nnue_weights_gen8.bin"
    os.makedirs("data", exist_ok=True)
    export_xrnn(model, output_weights_path)
    print("============================================================", flush=True)
    print("✅ HOÀN TẤT HUẤN LUYỆN PYTORCH NNUE GEN 8 QUY MÔ 100M FEN!", flush=True)
    print("============================================================", flush=True)
